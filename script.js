document.addEventListener('DOMContentLoaded', function() {
    ...
    const animateElements = document.querySelectorAll('.signature-card, .menu-item-card, .value-card, .testimonial-card, .team-member, .faq-item');
    ...
    if (whatsappBtn) {
        ...
        setInterval(() => {
            ...
            setTimeout(() => {
                whatsappBtn.style.animation = 'pulse 1.5s ease-in-out';
            }, 10);
        }, 5000);
    }
    ...
    console.log('🔥 Meat House website loaded successfully!');
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Meat House משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: 'f7ca0841-5145-4c47-9db7-6d6bef210160',
                    name: data.name || '',
                    email: data.email || '',
